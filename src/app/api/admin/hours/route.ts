...
export async function GET() {
  try {
    ...
    const { data, error } = await s.from('business_hours').select('*').order('weekday')
...
export async function POST(req: Request) {
  try {
    ...
    console.log('POST /api/admin/hours payload =', j(body))
    ...
    const clean = rows.map((r:any)=>({
      ...
      start_time: r.start_time,  // 'HH:MM' or 'HH:MM:SS'
      end_time:   r.end_time
    }))

    const s = supabaseAdmin()
    const { data, error } = await s.from('business_hours').upsert(clean, { onConflict: 'id' }).select('*')
    if (error) throw error
    return NextResponse.json({ ok: true, data })
  } catch (err:any) {
    console.error('POST /api/admin/hours error:', err)
    return NextResponse.json({ error: err?.message || String(err) }, { status: 500 })
  }
}
...
export async function DELETE(req: Request) {
  try {
    const body = await req.json().catch(()=>null)
    console.log('DELETE /api/admin/hours payload =', j(body))
    const ids:number[] = Array.isArray(body?.ids) ? body.ids : body?.id ? [body.id] : []
    if (!ids.length) throw new Error('No id(s)')
    const s = supabaseAdmin()
    const { error } = await s.from('business_hours').delete().in('id', ids)
    if (error) throw error
    return NextResponse.json({ ok: true })
  } catch (err:any) {
    console.error('DELETE /api/admin/hours error:', err)
    return NextResponse.json({ error: err?.message || String(err) }, { status: 500 })
  }
}
